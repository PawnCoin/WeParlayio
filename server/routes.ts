...
#WeParlay #BettingTech #SportsBox`
  ...
  app.post("/api/payments/cashapp", isAuthenticated, async (req, res) => {
    try {
      ...
      const { cashAppService } = await import('./services/cashAppService');
      
      const result = await cashAppService.processPayment({
        amount: parseFloat(amount),
        currency: 'USD',
        userId,
        description: description || 'WeParlay betting deposit'
      });

      if (result.success) {
        // Update user balance after successful Cash App payment
        await storage.updateUserBalance(userId, amount);
        ...
        res.json({
          success: true,
          message: `$${amount} deposited via Cash App`,
          paymentId: result.paymentId
        });
      } else {
        res.status(400).json({
          success: false,
          message: result.error || 'Cash App payment failed'
        });
      }
    } catch (error: any) {
      res.status(500).json({ message: "Cash App payment error: " + error.message });
    }
  });

  // Cash App Withdrawal
  app.post("/api/withdrawals/cashapp", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!user || user.balance < amount) {
        return res.status(400).json({ 
          message: `Insufficient funds. You have $${user?.balance || 0} but need $${amount}` 
        });
      }
      ...
      // Verify Cash App tag
      const cashTagVerification = await cashAppService.verifyCashTag(cashTag);
      if (!cashTagVerification.valid) {
        return res.status(400).json({ 
          message: "Invalid Cash App tag. Please check your $CashTag" 
        });
      }

      // Process withdrawal
      const result = await cashAppService.processWithdrawal({
        amount: parseFloat(amount),
        cashTag,
        userId
      });
      ...
      if (result.success) {
        ...
        res.json({
          success: true,
          message: `$${amount} withdrawn to ${cashTag}`,
          transferId: result.transferId
        });
      } else {
        res.status(400).json({
          success: false,
          message: result.error || 'Cash App withdrawal failed'
        });
      }
    } catch (error: any) {
      res.status(500).json({ message: "Cash App withdrawal error: " + error.message });
    }
  });
