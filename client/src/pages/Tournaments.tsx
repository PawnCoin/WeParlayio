...
export default function Tournaments() {
  ...
  const { data: scheduleResponse } = useQuery<any>({ queryKey: ["/api/events/today"], refetchInterval: 30000 });
  const games = Array.isArray(scheduleResponse?.events) ? scheduleResponse.events : [];
  ...
  return <div className="mx-auto max-w-7xl space-y-5 p-4">
    ...
    <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-3 lg:col-span-2">{tournament.events.map((storedEvent: any, index: number) => { const live = games.find((game: any) => String(game.id) === storedEvent.id) || {}; const event = { ...storedEvent, status: live.status || storedEvent.status, statusDetail: live.statusDetail || storedEvent.statusDetail, homeScore: live.homeTeam?.score ?? storedEvent.homeScore, awayScore: live.awayTeam?.score ?? storedEvent.awayScore }; return <Card key={event.id} className="cursor-pointer hover:border-emerald-500" onClick={() => setSelectedMatch(event)}><CardContent className="p-4"><div className="flex justify-between"><div><Badge variant={event.status === "live" ? "destructive" : "secondary"}>{event.status || "upcoming"}</Badge><h3 className="mt-2 font-bold">Match {index + 1}: {event.awayTeam} at {event.homeTeam}</h3><p className="text-xs text-muted-foreground">{event.sport} · {new Date(event.startTime).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })} · Score {event.awayScore ?? "–"}:{event.homeScore ?? "–"}{event.statusDetail ? ` · ${event.statusDetail}` : ""}</p></div><Link href="/live-tv" onClick={click => click.stopPropagation()}><Button size="sm" variant="outline"><Radio className="mr-1 h-4 w-4" />Watch live</Button></Link></div>{currentEntry && <div className="mt-3 grid grid-cols-2 gap-2">{[event.awayTeam, event.homeTeam].map((team: string) => <Button key={team} disabled={locked || currentEntry.status === "submitted"} variant={(currentEntry.picks?.[event.id] || picks[event.id]) === team ? "default" : "outline"} onClick={click => { click.stopPropagation(); setPicks(value => ({ ...value, [event.id]: team })); }}>{team}</Button>)}</div>}</CardContent></Card>; })}{currentEntry?.status !== "submitted" && currentEntry && <Button className="w-full" disabled={locked || Object.keys(picks).length !== tournament.events.length} onClick={() => mutate.mutate({ method: "PUT", url: "/api/tournaments/" + tournament.id + "/picks", data: { picks } })}>{locked ? "Predictions locked" : "Submit all predictions"}</Button>}</div>
